...
cardDate.addEventListener("input", function () {
  ...
  if (valueOfDate.length >= 1) {
    const formattedDate = valueOfDate
      .replace(/[^0-9]/g, "")
      .match(/.{1,2}/g)
      .join("/");

    const limitedString = formattedDate
      .substring(0, 6)
      .replace(/\/[^\/]*\//g, "/");
    let acu = "";
...
button.addEventListener("click", function (event) {
  ...
  if (cardNameText === "") {
    ...
    cardName.style.borderRadius = "2px";
    cardName.style.outlineColor = "red";
    hasErrorName = true;
  ...
  if (cardNumberText === "") {
    ...
    cardNumber.style.borderRadius = "2px";
  ...
  if (manyWordsDate >= 5) {
    ...
    cardDate.style.border = "1px solid black";
    cardDate.style.borderRadius = "2px";
  } else {
    erroName3.style.display = "flex";
    cardDate.style.border = "1px solid red";
    cardDate.style.borderRadius = "2px";
    cardDate.style.outlineColor = "red";
    hasErrorDate = true;
  }
  // Verificação de Erro do CVC
  if (manyWordsCVC >= 3) {
    erroName4.style.display = "none";
    cardCVC.style.border = "1px solid black";
    cardCVC.style.borderRadius = "2px";
  } else {
    erroName4.style.display = "flex";
    cardCVC.style.border = "1px solid red";
    cardCVC.style.borderRadius = "2px";
    cardCVC.style.outlineColor = "red";
    hasErrorCVC = true;
  }
